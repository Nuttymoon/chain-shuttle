// From https://github.com/ava-labs/avalanche-docs/blob/master/scripts/make_accounts.js
...
let ava = new avalanche.Avalanche("127.0.0.1", 9650, "http", 12345);
...
let memo = bintools.stringToBuffer(
...
async function exportTxXtoC() {
  ...
  let unsignedTx = await xchain.buildExportTx(
    utxoSet,
    amount,
    cChainBlockchainID,
    cAddressStrings,
...
async function importTxXtoC(evmAddr) {
  ...
  intxo.forEach((utxo) => {
    ...
    txid = utxo.getTxID();
    outputidx = utxo.getOutputIdx();
    output = utxo.getOutput();
    amt = output.getAmount().clone();
    input = new avalanche.evm.SECPTransferInput(amt);
    input.addSignatureIdx(0, cAddresses[0]);
    xferin = new avalanche.evm.TransferableInput(
      txid,
      outputidx,
      assetID,
      input
    ...
    evmOutput = new avalanche.evm.EVMOutput(cHexAddress, amt, assetID);
    evmOutputs.push(evmOutput);
  });

  let importTx = new avalanche.evm.ImportTx(
    12345,
...
module.exports = async (callback) => {
  ...
  for (var i = 0; i < maxAccounts; i++) {
    await w3.eth.personal.newAccount();
  }
  ...
  for (var i = 0; i < accounts.length; i++) {
    console.log(await w3.eth.getBalance(accounts[i]));
  }
};
