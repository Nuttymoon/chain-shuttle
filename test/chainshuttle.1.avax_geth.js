...
(function() {
  ...
  contract('ChainShuttle - init shuttle (dest chain)', function(accounts) {
    ...
    depositNonce = 0;
    return it('init shuttle contract on dest chain', async function() {
      var acc, i, len, newBridgeAddress, newErc20HandlerAddress, newGenericHandlerAddress;
...
\t`);
      await bridge.adminSetGenericResource(bridgeConf.chains[1].opts.genericHandler, '0x000000000000000000000000000000f44be64d2de895454c3467021928e55e00', shuttle.address, '0x00000000', web3.eth.abi.encodeFunctionSignature('offloadShuttle(bytes)'));
      for (i = 0, len = accounts.length; i < len; i++) {
        acc = accounts[i];
        await taxi.mint(acc, 1000000);
      }
      // Setup ChainShuttle to enable transfers
      await shuttle.setUpBridge(bridgeConf.chains[1].opts.bridge, bridgeConf.chains[1].opts.erc20Handler, '0x000000000000000000000000000000e389d61c11e5fe32ec1735b3cd38c69500', bridgeConf.chains[1].opts.genericHandler, '0x000000000000000000000000000000f44be64d2de895454c3467021928e55e00');
      ...
      return newGenericHandlerAddress.should.eql(bridgeConf.chains[1].opts.genericHandler);
    });
  });
